...
public class MainActivity extends BridgeActivity {
    ...
    private static final String OFFLINE_HTML = "<!DOCTYPE html><html><head>"
        ...
        + "</style></head><body>"
        + "<div class='logo'><div class='logo-box'><svg width='16' height='16' viewBox='0 0 24 24' fill='none'>"
        ...
        + "</svg></div><span class='logo-text'>CoFarmz</span></div>"
        + "<div class='badge'>No Internet</div>"
        + "<h1>Fields don't wait.<br><span>Neither should you.</span></h1>"
        + "<p>CoFarmz connects farmers, buyers and equipment — but right now your signal is on a tea break.</p>"
        + "<button onclick='Android.retry()'>Try Again</button>"
        + "</body></html>";
    ...
    public void onCreate(Bundle savedInstanceState) {
        // ✅ Set black background BEFORE super.onCreate so no white flash
        getWindow().getDecorView().setBackgroundColor(Color.parseColor("#1a2e1a"));
        ...
        // ✅ Also intercept WebView errors AFTER Capacitor loads
        // (handles case where network drops mid-load)
    ...
    private boolean isConnected() {
        try {
            ...
            return caps != null
                ...
                // ✅ VALIDATED means it actually confirmed internet access
        } catch (Exception e) {
            return false;
        }
    }
    ...
    private void showOfflinePage() {
        offlineWebView = new WebView(this);
        offlineWebView.setBackgroundColor(Color.parseColor("#1a2e1a"));
        offlineWebView.getSettings().setJavaScriptEnabled(true);

        // ✅ Add Android bridge so "Try Again" button works
        offlineWebView.addJavascriptInterface(new Object() {
            @android.webkit.JavascriptInterface
            public void retry() {
                runOnUiThread(() -> {
                    if (isConnected()) {
                        // Restart the activity cleanly
                        finish();
                        startActivity(getIntent());
                    } else {
                        offlineWebView.loadData(OFFLINE_HTML, "text/html", "UTF-8");
                    }
                });
            }
        }, "Android");
