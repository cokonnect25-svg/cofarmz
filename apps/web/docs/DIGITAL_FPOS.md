...
Admins and Super Admins can use **Export all FPOs PDF** for the full directory and its assigned farmers, or open a district and use **Export district PDF**. The report opens in a new tab; select **Save as PDF / Print**, then choose **Save as PDF** in the browser print dialog. Allow pop-ups for the site if prompted. All farmer pages are fetched through the existing authenticated reviewer APIs before the report is made available. Overall exports ignore directory search/status filters, include empty and inactive FPOs, and exclude farmers without an FPO assignment. Reports contain a detail block for each farmer: name, ID, email, phone number, saved address, district/state, gender, age, bio, latitude/longitude, email/phone verification, profile creation/update timestamps, and assignment status/source/reason. Missing values display as Not provided. Export requests use details=true on the authenticated reviewer endpoint; credentials and authentication secrets are never selected. Responses are private and non-cacheable. Reports remain grouped by district/state with generation time and totals. Browser printing preserves local-language text and keeps farmer blocks together where space allows.
