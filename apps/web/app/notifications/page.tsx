...
function NotificationsContent() {
  ...
  useEffect(() => {
    ...
    const announcement = notifications.find(
      (item) =>
        item.type === "announcement" && item.id === `ann-${announcementId}`
    );
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      const [notifRes, followRes] = await Promise.all([
        ...
        fetch(
          getApiUrl(`/api/follows?user_id=${user.id}&type=pending_requests`)
        ),
      ]);
    ...
    try {
      ...
      if (res.ok) {
        setFollowRequests((prev) =>
          prev.filter((r) => r.user_id !== followerId)
        );
      }
    } catch (e) {
      console.error(e);
              ...
              {followRequests.map((req) => (
                ...
                >
                  <img
                    ...
                    onClick={() =>
                      router.push(`/farmer-profile?id=${req.user_id}`)
                    }
                  />
                  <div className="flex-1 min-w-0">
                    <p
                  ...
                  <div className="flex gap-2 flex-shrink-0">
                    <button
                      onClick={() =>
                        handleFollowAction(req.user_id, "rejected")
                      }
                    ...
                    <button
                      onClick={() =>
                        handleFollowAction(req.user_id, "accepted")
                      }
                      disabled={processingFollow === req.user_id}
                  ...
                  <div className="flex-shrink-0">
                    {notif.type === "admin_reel" && notif.image ? (
                      ...
                      <div className="relative">
                        ...
                        />
                        <div
                          className={`absolute -bottom-1 -right-1 w-5 h-5 rounded-full ${getNotifBg(
                            notif.type
                      ...
                      <div
                        className={`w-11 h-11 rounded-full ${getNotifBg(
                          notif.type
                          ...
                          {notif.matchedCrops.slice(0, 3).map((crop) => (
                            <button
                              ...
                              onClick={(event) => {
                                ...
                                router.push(
                                  `/nearby-farmers?${params.toString()}`
                                );
                              }}
