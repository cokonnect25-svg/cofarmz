...
export async function GET(
  ...
  const comments =
    await sql`SELECT c.id, c.user_id, c.comment, c.created_at, c.parent_comment_id, u.name, u.image,
    ...
    WHERE c.post_id = ${Number(
      id
    )} GROUP BY c.id,u.name,u.image ORDER BY c.created_at ASC`;
...
export async function POST(
  ...
  const parent = parentCommentId
    ? await sql`SELECT user_id FROM profile_post_comments WHERE id=${Number(
        parentCommentId
      )} AND post_id=${Number(id)} LIMIT 1`
    : [];
  if (parentCommentId && !parent.length)
    return NextResponse.json(
      { error: "Reply target not found" },
      { status: 404 }
    );
  const inserted =
    await sql`INSERT INTO profile_post_comments (post_id, user_id, comment, parent_comment_id) VALUES (${Number(
      id
    )}, ${String(userId)}, ${clean}, ${
      parentCommentId ? Number(parentCommentId) : null
    }) RETURNING id, user_id, comment, created_at, parent_comment_id`;
  const users = await sql`SELECT name, image FROM "user" WHERE id = ${String(
    userId
  )} LIMIT 1`;
  const owners = await sql`SELECT user_id FROM profile_posts WHERE id=${Number(
    id
  )} LIMIT 1`;
  ...
  await addSocialNotification({
    ...
    type: parentCommentId ? "post_reply" : "post_comment",
    postId: id,
  ...
  await notifyMentions({
    ...
    postId: id,
    commentId: inserted[0].id,
    exclude: [String(recipient || "")],
  });
  return NextResponse.json(
    { ...inserted[0], ...users[0], likes: 0, is_liked: false },
    { status: 201 }
  );
}
...
export async function DELETE(
  ...
  await ensureSocialActivityTables();
  const deleted =
    await sql`DELETE FROM profile_post_comments WHERE id=${commentId} AND post_id=${Number(
      id
  ...
  await sql`DELETE FROM social_activity_notifications WHERE post_id=${Number(
    id
