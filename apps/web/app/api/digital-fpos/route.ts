...
import { requireActor, isSuperAdmin, fpoError, FpoError } from '@/lib/fpo-access';
...
export async function GET(request: Request) {
  try {
    ...
    const rows = await sql`SELECT f.*,d.state,d.district,g.id AS group_id,f.name AS group_name,
      (SELECT count(*)::int FROM farmer_fpo_assignments a JOIN "user" u ON u.id=a.farmer_id WHERE a.group_id=g.id AND can_receive_fpo_message(u.id,g.id)) AS farmer_count
      FROM digital_fpos f JOIN fpo_districts d ON d.id=f.district_id
      JOIN farmer_groups g ON g.digital_fpo_id=f.id
      WHERE f.status='active' OR ${isSuperAdmin(actor.role)} ORDER BY d.state,d.district`;
    ...
    return NextResponse.json({ fpos: rows, mine: mine || null, can_manage: isSuperAdmin(actor.role) });
